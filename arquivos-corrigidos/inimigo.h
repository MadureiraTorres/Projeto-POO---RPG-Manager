 ...
 *
 * BUGS CORRIGIDOS:
 *  1. IACautelosa usava rand() sem srand() — a sequência de aleatoriedade era
 *     sempre idêntica a cada execução do programa (seed padrão = 0).
 *     Corrigido: srand(time(nullptr)) chamado uma única vez via flag estática
 *     dentro de agir(), garantindo seed diferente por execução sem poluir o
 *     escopo global.
 */
#ifndef INIMIGO_H
#define INIMIGO_H
...
class IACautelosa : public ComportamentoIA {
    public:
        ...
        void agir(Entidade &self, Entidade &alvo) override {
            // FIX: inicializa a seed uma única vez por processo
            static bool seedIniciada = false;
            if(!seedIniciada) {
...
class Inimigo : public Entidade {
    ...
    public:
        Inimigo(const string &nm, int hp, int lv, int exp, int almas, ComportamentoIA* comportamento)
            : Entidade(nm, hp, lv), expRecompensa(exp), almasRecompensa(almas), ia(comportamento) {}
