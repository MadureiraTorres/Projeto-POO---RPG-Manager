 ...
 *
 * BUGS CORRIGIDOS:
 *  1. listarSaves() era declarado const, mas sqlite3_prepare_v2() e
 *     sqlite3_step() modificam o estado interno do handle sqlite3* — o que
 *     viola const-correctness e causa comportamento indefinido em compiladores
 *     restritivos. Corrigido: membro db declarado como mutable sqlite3*, e
 *     listarSaves() permanece const de forma correta.
 */
...
class Persistencia {
    private:
        // FIX: mutable permite que métodos const (listarSaves, carregarInfo)
        // usem a API do SQLite, que modifica o handle internamente.
        mutable sqlite3* db;
        string           caminhoDB;
    ...
    public:
        ...
        bool carregarInfo(const string &nome,
                          ...
                          int &outForca, int &outDex, int &outExp) const {
